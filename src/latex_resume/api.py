...
# ---------------------------------------------------------------------------
# Pydantic request/response models live in schemas.py; re-exported for routers.
from latex_resume.schemas import (  # noqa: E402,F401
    _PREFILL_LABELS,
    OptimizeRequest,
    RerenderRequest,
    UploadResponse,
...
# Limits apply per authenticated profile (else per declared profile, else per IP),
# so one noisy user cannot exhaust the LLM budget for everyone behind a NAT.
...
async def _get_session_or_404(session_id: str, profile_id: str | None = None) -> ResumeSession:
    """Return the classic session, hiding sessions another profile opened."""
    session = await store.get(session_id)
    if session is None:
        raise HTTPException(404, f"Session '{session_id}' not found or expired.")
    if profile_id is not None and session.profile_id and session.profile_id != profile_id:
        raise HTTPException(404, f"Session '{session_id}' not found or expired.")
    return session
