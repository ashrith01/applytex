...
def create_lab(db_path: Path) -> FastAPI:
    ...
    async def isolate_lab(request: Request, call_next: Callable[[Request], Awaitable[Response]]) -> Response:
        ...
        allowed = path.startswith(("/lab", "/profile", "/applications", "/apply-runs", "/extension/forms", "/extension/jobs/capture", "/extension/resume/prepare", "/health")) or (path == "/auth/status" and request.method == "GET") or (path.startswith("/jobs/") and request.method == "GET")
